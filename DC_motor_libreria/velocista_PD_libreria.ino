...
void setup()
{
    ...
    derecha.begin(MOTOR_DER_DIR, MOTOR_DER_VEL);
    izquierda.begin(MOTOR_IZQ_DIR, MOTOR_IZQ_VEL);
...
void loop()
{
    ...
    case CARRERA:
        ...
        if (bigButtonPulsed() == true)
        {
            ...
            derecha.stop();  // detiene los dos motores
            izquierda.stop();
        ...
        for (int i = 0; i < 4; i++)
        {
            if (bitRead(values, i) == HIGH)
            {                             // si el sensor ha detectado negro,
        ...
        if (num_negros != 0)
        {                        // sólo si se ha detectado algún negro,
        ...
        if (velocidad_der < 0)
        { // se comprueba que la velocidad esté dentro del rango de 0 a 255.
            velocidad_der = 0;
        }
        ...
        derecha.move(FORWARD, velocidad_der);
        izquierda.move(FORWARD, velocidad_izq);

        break;
    }
    delay(TASA_REFRESCO); // tiempo de espera hasta la próxima aplicación del control PD
}
